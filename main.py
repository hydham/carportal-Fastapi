...
def edit_car(request: Request, id: int = Query(...)):
    car = cars.get(id)
    if not car:
        return templates.TemplateResponse("/search.html", {"request": request, "car": car, "title": "Edit Car", "id": id},
                                          status_code=status.HTTP_404_NOT_FOUND)
    return templates.TemplateResponse("/edit.html", {"request": request, "car": car, "title": "Edit Car", "id": id})

@app.post("/cars/{id}")
def update_car(request: Request,
            id: int, 
    ...
    stored = cars.get(id)
    if not stored:
        return templates.TemplateResponse("/search.html", {"request": request, "car": stored, "title": "Edit Car", "id": id},
                                          status_code=status.HTTP_404_NOT_FOUND)
...
def delete_car(request: Request, id: int=Path(...)):
    if not cars.get(id):
        return templates.TemplateResponse("/search.html", {"request": request, "id": id, "title": "Search Car"}, 
                                          status_code= status.HTTP_404_NOT_FOUND)
    del cars[id]
    return RedirectResponse(url="/cars")